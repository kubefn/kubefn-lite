/**
 * KubeFn Lite — Durable Event Bus Plugin for OpenClaw
 *
 * Bridges to the Python KubeFn LiteEngine via persistent subprocess
...
const BRIDGE_SCRIPT = path.join(PLUGIN_DIR, "kubefn_engine", "bridge.py");
...
function ensureProcess(): ChildProcess {
  ...
  proc.on("close", (code: number | null) => {
    ...
    for (const [rid, { reject }] of _pendingRequests) {
      reject(new Error(`KubeFn Lite bridge exited with code ${code}`));
    }
    ...
    try {
      ...
      setTimeout(() => {
        if (_pendingRequests.has(requestId)) {
          ...
          reject(new Error(`KubeFn Lite bridge timeout for command: ${command}`));
        }
      }, 30000);
...
export async function onStartup(context: {
  ...
  try {
    ...
    if (result.healthy) {
      ...
      console.log(
        `[kubefn] Event bus ready: ${stats.total_events ?? 0} events, ${stats.consumer_groups ?? 0} groups`
      );
    } else {
      console.error(`[kubefn] Health check failed: ${result.error}`);
    }
  } catch (err) {
    console.error(`[kubefn] Startup error: ${(err as Error).message}`);
  }
}

export async function onShutdown(): Promise<void> {
  killProcess();
  console.log("[kubefn] Event bus shut down");
}
